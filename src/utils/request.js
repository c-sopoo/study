...
/* 普通请求 */
export const request = (url, params = {}, config = {}, autoErrorRes = true, autoErrorData = true, autoCancle = true) => {
  if (autoCancle) { // 离开路由时是否自动取消请求
  ...
  if (!['put','post','patch'].includes(args.method.toLowerCase())) {
    ...
    args['paramsSerializer'] = args['paramsSerializer'] || function () {
      return qs.stringify(params, { arrayFormat: 'indices' })
    }
  }

  return axiosInstance(args).then(res => {
    // console.log(res)
    // 未登录
    ...
    const errStatus = err.response.status || -100
    if (autoErrorRes && err.message) {
...
export const sessionRequest = (url, params = {}, config = {}, outTime = -1, autoErrorRes = true, autoErrorData = true, autoCancel = true) => {
  ...
  if (itemValue) {
    ...
    const overTime = nowTime - itemValue.lastTime
    if (outTime<0 || overTime < outTime *1000) {
      return Promise.resolve(itemValue.data)
    }
  }
